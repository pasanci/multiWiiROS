...
{
    declare_parameter("device_path", "/dev/ttyS0");
    declare_parameter("baud", 115200);
    for(const std::string& sub_param : sub_params) {
        declare_parameter<float>("sub/"+sub_param,0.01);
    }
	...
	ser_arm = this->create_service<mavros_msgs::srv::CommandBool>("arm", std::bind(&MultiWiiNode::arm, this,
                                std::placeholders::_1, std::placeholders::_2));
	ser_force_arm = this->create_service<mavros_msgs::srv::CommandBool>("force_arm", std::bind(&MultiWiiNode::force_arm, this,
                                std::placeholders::_1, std::placeholders::_2));
    ...
    set_parameter(rclcpp::Parameter("sub/imu", rclcpp::ParameterValue (0.01)));       // 102
    set_parameter(rclcpp::Parameter("sub/motor", rclcpp::ParameterValue (0.01)));      // 104
    set_parameter(rclcpp::Parameter("sub/rc", rclcpp::ParameterValue (0.01)));         // 105
    set_parameter(rclcpp::Parameter("sub/attitude", rclcpp::ParameterValue (0.01)));   // 108
    set_parameter(rclcpp::Parameter("sub/altitude", rclcpp::ParameterValue (0.01)));   // 109
    set_parameter(rclcpp::Parameter("sub/analog", rclcpp::ParameterValue (0.01)));     // 110
    set_parameter(rclcpp::Parameter("sub/voltage", rclcpp::ParameterValue (0.01)));      // 128
    set_parameter(rclcpp::Parameter("sub/current", rclcpp::ParameterValue (0.01)));      // 129
    set_parameter(rclcpp::Parameter("sub/battery", rclcpp::ParameterValue (0.01)));      // 130
}
...
void MultiWiiNode::rc_override_raw(const mavros_msgs::msg::OverrideRCIn::SharedPtr rc) {
    ...
    for(const uint16_t c : rc->channels) {
		channels.push_back(c);
	}
    fcu->setRc(channels);
}

void MultiWiiNode::arm(const std::shared_ptr<mavros_msgs::srv::CommandBool::Request> request, std::shared_ptr<mavros_msgs::srv::CommandBool::Response> response){
	if(request->value){
		...
		if (fcu->setRc(channels)) {
			// rc data has been sent
			const int RUNTIME = 3;
...
void MultiWiiNode::force_arm(const std::shared_ptr<mavros_msgs::srv::CommandBool::Request> request, std::shared_ptr<mavros_msgs::srv::CommandBool::Response> response){
	if(request->value){
		// spin motors 1 to 4
		//const std::array<uint16_t, msp::msg::N_MOTOR> &motor_values
		std::array<uint16_t, msp::msg::N_MOTOR> motor_values;
		motor_values = {1100, 1100, 1100, 1100, 0, 0, 0, 0};
		if (fcu->setMotors(motor_values)) {
			// rc data has been sent
			response->success = true;
		}
		else{
		...
		// stop motors
		if (fcu->setMotors({{1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000}})) {
			// rc data has been sent
			response->success = true;
		}
		else{
