...
Client::Client() : port(io), log_level_(SILENT), msp_ver_(1), fw_variant(FirmwareVariant::INAV) {}
...
FirmwareVariant Client::getVariant() const {
	return fw_variant;
}
...
bool Client::connectPort(const std::string& device, const size_t baudrate) {
    try {
        ...
        throw std::runtime_error("Error when opening '" + device +  "': " + e.code().category().message(ecode) + " (error code: " + std::to_string(ecode) + ")");
    }
    return isConnected();
}
...
bool Client::startReadThread() {
    ...
    thread = std::thread([this] {
        asio::async_read_until(port, buffer, std::bind(&Client::messageReady, this, std::placeholders::_1, std::placeholders::_2), std::bind(&Client::processOneMessage, this, std::placeholders::_1, std::placeholders::_2));
        io.run();
    });
    return true;
}
...
bool Client::sendMessage(msp::Message& message, const double& timeout) {
    if(log_level_ >= DEBUG){
        std::cout << "sending message - ID " << size_t(message.id()) << std::endl;
	}
    ...
    const auto predicate = [&] {
        mutex_response.lock();
        const bool received = (request_received != nullptr) && (request_received->id == message.id());
    ...
    if(timeout > 0) {
        if(!cv_response.wait_for(lock, std::chrono::milliseconds(size_t(timeout * 1e3)), predicate)) {
            if(log_level_ >= INFO){
                std::cout << "timed out waiting for response to message ID " << size_t(message.id()) << std::endl;
			}
            return false;
        }
    }
...
bool Client::sendMessageNoWait(const msp::Message& message) {
    if(log_level_ >= DEBUG){
        std::cout << "async sending message - ID " << size_t(message.id()) << std::endl;
	}
    if(!sendData(message.id(), message.encode())) {
        if(log_level_ >= WARNING){
...
uint8_t Client::extractChar() {
    if(buffer.sgetc() == EOF) {
        if(log_level_ >= WARNING){
            std::cerr << "buffer returned EOF; reading char directly from port" << std::endl;
		}
...
bool Client::sendData(const msp::ID id, const ByteVector& data) {
    ...
    if(log_level_ >= DEBUG){
        std::cout << "write complete: " << bytes_written << " vs " << msg.size() << std::endl;
	}
    return (bytes_written == msg.size());
}

ByteVector Client::packMessageV1(const msp::ID id, const ByteVector& data) const {
    ByteVector msg;
...
ByteVector Client::packMessageV2(const msp::ID id, const ByteVector& data) const {
    ByteVector msg;
    msg.push_back('$');                           // preamble1
...
void Client::processOneMessage(const asio::error_code& ec, const std::size_t& bytes_transferred) {
    if(log_level_ >= DEBUG){
        std::cout << "processOneMessage on " << bytes_transferred << " bytes" << std::endl;
	}
    ...
    if(msg_marker != '$'){
        std::cerr << "Message marker " << size_t(msg_marker) << " is not recognised!" << std::endl;
	}
    ...
    if(ver == 0) {
        std::cerr << "Version marker " << size_t(ver_marker) << " is not recognised!" << std::endl;
    }
...
std::pair<iterator, bool> Client::messageReady(iterator begin, iterator end) const {
    iterator i = begin;
    ...
    if(*i == '$' && *(i + 1) == 'M') {
        ...
        const uint16_t payload_size = uint8_t(*(i + 6)) | uint8_t(*(i + 7) << 8);
...
ReceivedMessage Client::processOneMessageV2() {
    ...
    const uint8_t dir = extractChar();
    if(log_level_ >= DEBUG){
		std::cout << "dir: " << size_t(dir) << std::endl;
	}
    const bool ok_id = (dir != '!');
    ...
    if(log_level_ >= DEBUG){
		std::cout << "flag: " << size_t(flag) << std::endl;
	}
    ...
    ret.id                = msp::ID(id);
    if(log_level_ >= DEBUG){
		std::cout << "id: " << size_t(id) << std::endl;
	}
